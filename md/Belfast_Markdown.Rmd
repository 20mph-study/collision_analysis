...
leaflet(belfast_road_data) %>% addTiles() %>%  addPolylines(group = 'roads') %>%
  ...
  addCircleMarkers(data = total,lng = total$X.1,lat=total$Y.1, group = 'incidents linked to a road') %>% 
  addMarkers(lng = total$X, lat = total$Y, 
             popup = paste("Accident index: ", total$a_ID, "<br>",
                           "Speed limit: ", total$a_speed ), group = 'incidents') %>% addLayersControl(overlayGroups = c('roads', 'incidents','incidents linked to a road'),
...
write.csv(updated_dataset, file = paste0(dir_path, "/Belfast/final_Belfast_data.csv"),row.names=FALSE)
