...
#leaflet(belfast_road_data) %>% addTiles() %>%  addPolylines( ) %>%
#  setView(lng = -5.926437 , lat = 54.607868, zoom = 11) %>%
#  addMarkers(data = total,lng = total$X,lat=total$Y)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
belf_annual_table(total_zones,total,pre,post)
```
...
e <- c(nrow(total_zones[total_zones$a_date >= "2016-02-02" & total_zones$a_date <= "2017-12-31", ]))
d <- c(nrow(total_zones[total_zones$a_date >= "2018-01-01" & total_zones$a_date <= "2019-03-31", ]))
...
Y_2014_15 <- c(b,b," "," ")
Y_2015_16 <- c(c,c," "," ")
Y_2016_17 <- c(e,round(e/1.9,digits = 2)," "," ")
Y_2018_19 <- c(d,round(d/1.25 ,digits = 2)," "," ")
