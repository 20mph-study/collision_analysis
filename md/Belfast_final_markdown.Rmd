...
belfast_road_data <- readOGR(paste0(dir_path, 'Belf shapefiles/20mph Speed Limit Streets/20mph_Speed_Limit_Streets.shp'), verbose = FALSE)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#Output Areas 
gdb_path_out <- paste0(dir_path, "Belf shapefiles/OA_ni-ESRI_format/OA_ni.shp")
...
#READ DATA
#PSNI data
belf_data <- read_csv( paste0(dir_path, "Belf dataset/Belfast/Belfast_data.csv"))
belf_data <- delete_na(belf_data,c("X","Y"))
belf_data <- belf_data %>% mutate(year = format(as.Date(belf_data$a_date, format="%m/%d/%Y"),"%Y"))

belf_control_data <- read_csv( paste0(dir_path, "Belf dataset/Belfast/Belfast_control_data.csv"))
