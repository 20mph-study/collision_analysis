...
belfast_road_data <- readOGR(paste0(dir_path, '20mph study collisions\\20mph Speed Limit Streets\\20mph_Speed_Limit_Streets.shp'), verbose = FALSE)
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
#Output Areas 
gdb_path_out <- paste0(dir_path, "20mph study collisions\\OA_ni-ESRI_format\\OA_ni.shp")
...
#READ DATA
#PSNI data
belf_data <- read_csv( paste0(dir_path, "20mph study collisions\\Belfast\\Belfast_data.csv"))
belf_data <- delete_na(belf_data,c("X","Y"))
belf_data <- belf_data %>% mutate(year = format(as.Date(belf_data$a_date, format="%m/%d/%Y"),"%Y"))

belf_control_data <- read_csv( paste0(dir_path, "20mph study collisions\\Belfast\\Belfast_control_data.csv"))
...
sum_pre <- pre[pre$a_speed == 20,2] + pre[pre$a_speed == 30,2] + pre[pre$a_speed == 40,2]
...
rate_pre_20mph <- round(collisions_pre_20mph / sum1, 3)
rate_post_20mph <- round(collisions_post_20mph / sum2, 3)
diff_in_rates <- rate_pre_20mph - rate_post_20mph
percentage_diff_in_rates <- round(diff_in_rates / rate_pre_20mph * 100, 3)
...
kable(df,caption = "Table 3: Effect estimate(GIS)") %>% kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
total <- total_zones[total_zones$a_date >="2019-01-01" & total_zones$a_date <= "2019-12-31", ]
leaflet(belf_impl_zones_out) %>% addTiles() %>% addPolygons() %>%
                         setView(lng = -5.926437 , lat = 54.607868, zoom = 14) %>% 
                          addMarkers(lng = total$X, lat = total$Y, 
                                      popup = paste("Accident index: ", total$a_ID, "<br>",
...
> Count accidents in the implementation zone
...
leaflet(belf_control_zones) %>% addTiles() %>% addPolygons() %>% addMarkers(lng = belf_control_data$X,  lat = belf_control_data$Y) %>% setView(lng = -5.69092, lat = 54.59236, zoom = 13)
...
buffer_data <-function(data,zone){
  ...
  points <- gBuffer(data, width=10, byid = TRUE )
  count <- over(points,zone,returnList = FALSE)
  return(nrow(unique(count)))
}

buffer_data(belf_control_data,belf_control_zones)
...
write.csv(updated_dataset, file = paste0(dir_path, "20mph study collisions\\cleaned datasets\\final_Belfast_data.csv"),row.names=FALSE)
