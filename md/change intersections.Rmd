...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
mergedDf <- data1[data1$X1st_Road_Class == "A", ]
network_main <- edin_cons_streets[edin_cons_streets@data$LAYER  %in% c("20mph main streets","30mph","Part time 20mph"),]
 
new_lines <- nearest_line(mergedDf,network_main,10)
new_nearest_road <- spTransform(new_lines,"+init=epsg:4326")
new_nearest_road <-new_lines@data
new_nearest_road <- unique(merge(new_nearest_road,network_main@data,by.x = "nearest_line_id" ,by.y = "ID"))
...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
mergedDf <- data1[data1$X1st_Road_Class == "B", ]
network_minor <- edin_cons_streets[edin_cons_streets@data$LAYER  %in% c("20mph existing streets","20mph local streets"),]
 
new_lines_minor <- nearest_line(mergedDf,network_minor,10)
minor_nearest_road <- spTransform(new_lines,"+init=epsg:4326")
minor_nearest_road <-new_lines@data
minor_nearest_road <- unique(merge(minor_nearest_road,network_minor@data,by.x = "nearest_line_id" ,by.y = "ID"))

df2 <- minor_nearest_road[,-c(38,41,42,44,46,48)]
```
...
```{r,echo=FALSE, warning=FALSE,message=FALSE}
mergedDf <- data1[data1$X1st_Road_Class == "C", ]
...
df3 <- minor_nearest_road[,-c(38,41,42,44,46,48)]
```
...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
mergedDf <- data1[data1$X1st_Road_Class == "Unclassified", ]
...
df4 <- minor_nearest_road[,-c(38,41,42,44,46,48)]
```
...
```{r}
network_20mphpartime <- edin_cons_streets[edin_cons_streets@data$LAYER  %in% c( "30mph","20mph local streets","20mph main streets","20mph existing streets"),]
mergedDf <- data1[data1$LAYER == c("Part time 20mph"), ]

new_lines_20mphexisting <- nearest_line(mergedDf,network_20mphpartime,10)
new_nearest_road_20mphexisting <- spTransform(new_lines_20mphexisting,"+init=epsg:4326")
new_nearest_road_20mphexisting <-new_lines_20mphexisting@data
new_nearest_road_20mphexisting <- unique(merge(new_nearest_road_20mphexisting,network_20mphexisting@data,by.x = "nearest_line_id" ,by.y = "ID"))

df9 <- new_nearest_road_20mphexisting[,-c(38,41,42,44,46,48)]
```
...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
changed <- rbind(df1,df2,df3,df4,df5,df6,df7,df8,df9)
