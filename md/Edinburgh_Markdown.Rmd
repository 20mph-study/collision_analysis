...
type <- c("Streets at 30mph","Streets at 20mph","Part time 20mph","Streets at 30mph changed to 20mph","All streets")
...
kable(df,caption = "Table 1: Average annual road traffic collision rates in the city of Edinburgh (council data)") %>% kable_styling(bootstrap_options = c("striped", "hover")) %>%  footnote(general = " ", number = "All accidents at 20mph, 30mph used in the table")
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
Tax_year <- c("Annual number of collisions","Rate"," "," ","Effect estimate ")
a <- sum(pre13[,2])
b <- sum(pre14[,2])
c <- sum(pre15[,2])
d <- sum(po[,2])
...
df <- data.frame(Tax_year,Y_2013_14,Y_2014_15,Y_2015_16,Y_2016_18,Y_2018_19)
colnames(df) <- c("Tax year", "2013/14","2014/15","2015/16","2016/17","2018/19")
...
kable(list(df,df_cal),caption = "Effect estimate", booktabs = TRUE, valign = 't') %>% kable_styling(bootstrap_options = c("hover","striped") )
