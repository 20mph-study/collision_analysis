...
mergedDf$Accident_Severity <- as.character(mergedDf$Accident_Severity)
mergedDf$Accident_Severity[mergedDf$Accident_Severity == 1] <- "Fatal collision"
mergedDf$Accident_Severity[mergedDf$Accident_Severity == 2] <- "Serious collision"
mergedDf$Accident_Severity[mergedDf$Accident_Severity == 3] <- "Slight collision"
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
pre_20 <- count_pre(mergedDf[mergedDf$Date >="2013-01-01" & mergedDf$Date <= "2015-12-31", ])
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
type <- c("Streets at 30mph","Streets at 20mph","Streets at 30mph changed to 20mph")
collisions_pre_20mph <- c(1017,526,2151)
collisions_post_20mph <- c(175,74,331)
rate_pre_20mph <- c("0.276" ,"0.142","0.582")
rate_post_20mph <- c("0.302" ,"0.128","0.57")
diff_in_rates <- c("-0.026" ,"0.014","0.012")
percentage_diff_in_rates <- c("-2.6 %" ,"1.4 %","1.2 %")
...
write.csv(updated_dataset, file ="V:\\Studies\\MOVED\\HealthImpact\\Data\\20mph study collisions\\final_Edinburgh_data.csv",row.names=FALSE)
```
