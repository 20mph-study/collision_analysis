...
df <- data
names <- data.frame(names(edin_road_data))
```
...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
leaflet_map(mergedDf[mergedDf$year == "2013", ],edin_cons_streets)
count(mergedDf[mergedDf$Date >= "2013-07-31" & mergedDf$Date <= "2014-07-30" ,])
aa <- mergedDf[mergedDf$Date >= "2013-07-31" & mergedDf$Date <= "2014-07-30" ,]
AA <- delete_na(mergedDf[mergedDf$Date >= "2013-07-31" & mergedDf$Date <= "2014-07-30" ,],"Accident_Index")
...
> Table: Average annual road traffic collision rates in the city of Edinburgh ( council data )
...
kable(df,caption = "Table 1: Average annual road traffic collision rates in the city of Edinburgh (council data)") %>% kable_styling(bootstrap_options = c("striped", "hover")) %>%  footnote(general = " ", number =c("All accidents at 20mph, 30mph","pre-20mph : 2013 / 2016 ( 3 years )","post-20mph : 2018 / 2019 ( 16 months )","diff in rates = pre - post / pre ")) %>% pack_rows("Streets not changed", 1, 3,label_row_css = "background-color: #e5f5e0") %>% pack_rows("Streets changed", 4, 4,label_row_css = "background-color: #e5f5e0") %>% pack_rows("All streets", 5, 5,label_row_css = "background-color:#e5f5e0 ;")
#e5f5e0
```

> Table: Average annual road traffic collision rates in the city of Edinburgh ( police )
...
kable(df,caption = "Table 2: Average annual road traffic collision rates in the city of Edinburgh (police)") %>% kable_styling(bootstrap_options = c("striped", "hover"))
```

> Effect estimate

```{r,echo=FALSE,warning=FALSE,message=FALSE}
Tax_year <- c("Annual number of collisions","Rate per year"," ","Effect estimate ")
a <- sum(pre13[,2])
b <- sum(pre14[,2])
c <- sum(pre15[,2])
d <- sum(po[,2])
...
colnames(df_cal) <-  c("Formula", " ")

kable(df,caption = "Effect estimate", booktabs = TRUE, valign = 't') %>% kable_styling(bootstrap_options = c("hover","striped") )
kable(df_cal,caption = "Calculations") %>% kable_styling(bootstrap_options = c("hover","striped") )
#round(unadjusted_estimate*100,digits = 2),round(secular_trend*100,digits = 2),
#"striped", "hover"
```
...
edin_rd <- rd %>% filter(rd$`Local_Authority_(District)` %in% ("923")) 

edin_road_data <- rbind.fill(edin_rd,edin_road_data)
control_zone_data <- rbind.fill(rd,control_zone_data)
...
total_zones$Date <- as.character(total_zones$Date)
...
s_13a <- count_impl(total[total$Date >= "2013-07-31" & total$Date <= "2016-07-31", ])
...
map_zones %>%
  ...
  addMarkers(lng =total$Longitude, lat = total$Latitude, 
             popup =paste("Accident index: ", total$Accident_Index, "<br>",
                          "Zone: ", total$ImplementationZone))
s_14 <- count_impl(total[total$Date >= "2014-01-01" & total$Date <= "2014-12-31", ])
``` 
...
map_zones %>% addMarkers(lng =total$Longitude, lat = total$Latitude, 
                         popup =paste("Accident index: ", total$Accident_Index, "<br>",
                                      "Zone: ", total$ImplementationZone))
s_15 <- count_impl(total[total$Date >= "2015-01-01" & total$Date <= "2015-12-31", ])
```
...
map_zones %>% addMarkers(lng =total$Longitude, lat = total$Latitude, 
                         popup =paste("Accident index: ", total$Accident_Index, "<br>",
                                      "Zone: ", total$ImplementationZone))
s_16 <- count_impl(total[total$Date >= "2016-01-01" & total$Date <= "2016-12-31", ])
```
...
map_zones %>% addMarkers(lng =total$Longitude, lat = total$Latitude, 
                         popup =paste("Accident index: ", total$Accident_Index, "<br>",
                                      "Zone: ", total$ImplementationZone))
s_17 <- count_impl(total[total$Date >= "2017-01-01" & total$Date <= "2017-12-31", ])
```
...
map_zones %>% addMarkers(lng =total$Longitude, lat = total$Latitude, 
                         popup =paste("Accident index: ", total$Accident_Index, "<br>",
                                      "Zone: ", total$ImplementationZone))
s_18 <- count_impl(total[total$Date >= "2018-01-01" & total$Date <= "2018-12-31", ])
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
s_pre <- total_zones[total_zones$Date >= "2013-07-31" & total_zones$Date <= "2016-07-31", ]
zone_1a <- length(s_pre[s_pre$ImplementationZone%in%"1",1])
zone_1b <- length(s_pre[s_pre$ImplementationZone%in%"7",1])

s_pre <- total_zones[total_zones$Date >= "2014-02-28" & total_zones$Date <= "2017-02-28", ]
zone_2 <- length(s_pre[s_pre$ImplementationZone%in%c("2"),1])
zone_3 <- length(s_pre[s_pre$ImplementationZone%in%"3",1])

s_pre <- total_zones[total_zones$Date >= "2014-08-16" & total_zones$Date <= "2017-08-16", ]
zone_4 <- length(s_pre[s_pre$ImplementationZone%in%"4",1])
zone_5 <- length(s_pre[s_pre$ImplementationZone%in%"5",1])

s_pre6 <-  total_zones[total_zones$Date >= "2015-03-05" & total_zones$Date <= "2018-03-05", ]
zone_6 <- length(s_pre6[s_pre6$ImplementationZone%in%"6",])
