...
kable(df,caption = "Table 1: Average annual road traffic collision rates in the city of Edinburgh (council data)") %>% kable_styling(bootstrap_options = c("basic"),font_size = 12 ) %>%  footnote(general = " ", number =c("All accidents at 20mph, 30mph","pre-20mph : 2013 / 2016 ( 3 years )","post-20mph : 2018 / 2019 ( 16 months )","diff in rates = pre - post / pre ")) %>% pack_rows("Streets stayed", 1, 3,label_row_css = "background-color: #e5f5e0 ;") %>% pack_rows("Streets changed", 4, 4,label_row_css = "background-color: #e5f5e0;") %>% pack_rows("All streets", 5, 5,label_row_css = "background-color:#e5f5e0 ;")
#e5f5e0
```
...
kable(df,caption = "Effect estimate", booktabs = TRUE, valign = 't') %>% kable_styling(position = "center",bootstrap_options = c("hover","striped"),font_size = 20 )
kable(df_cal,caption = "Calculations") %>% kable_styling(position = "center",bootstrap_options = c("basic"),font_size = 20 )
```
