...
#leaflet(edin_aclass_streets) %>% addTiles() %>% addPolygons()
```
...
#inter <- intersect(Aa,edin_road_network)
...
#Read shapefile
#edin_minorclass_streets <- readOGR(dsn = gdb_path,layer="minor_rd_polyline", verbose = FALSE)

#library(raster)
#x <- bind(edin_aclass_streets, edin_bclass_streets, edin_minorclass_streets)
#Transform to long/lat
...
subs_union <- union(x,edin_cons_streets)
leaflet(subs_union) %>% addTiles() %>% addPolygons()
```
...
nearest_road <- nearest_line(edin_road_data,inter,10)
nearest_road <- spTransform(nearest_road,"+init=epsg:4326")
head(nearest_road[,c(2,37,38)])
mergedDf <- unique(merge(data.frame(nearest_road),inter@data,by.x = "nearest_line_id" ,by.y = "ID"))
```
