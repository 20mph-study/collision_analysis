...
write.csv(mergedDf, file = paste0(dir_path, "/mergedDf.csv"),row.names=FALSE)
mergedDf <- read_csv(paste0(dir_path, "/mergedDf.csv"))
mergedDf <- mergedDf[,-c(36:40,42:46)]
```
...
con_data <- con_data[con_data$Date >= "2014-02-28" & con_data$Date <= "2018-12-31", ]
...
con_data <- con_data[con_data$Date >= "2014-08-16" & con_data$Date <= "2018-12-31", ]
...
con_data <- control_zone_data %>% filter(control_zone_data$`Local_Authority_(District)` %in% c("935"))
con_data <- con_data[con_data$Date >= "2015-03-05" & con_data$Date <= "2018-12-31", ]
