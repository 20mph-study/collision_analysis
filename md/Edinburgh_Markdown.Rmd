...
```{r, echo=FALSE, warning=FALSE,message=FALSE}
line <- edin_road_network
points <- edin_road_data[edin_road_data$Date >= "2013-08-01" & edin_road_data$Date <= "2013-09-04",]
tolerance <- 10

nearest_road <- nearest_line(points,line,tolerance)
nearest_road <- spTransform(nearest_road,"+init=epsg:4326")

# calculate network distance along our line to each snapped point
dists <- nearest_road$snap_dist

# we want to recursively do the following:
# 1. calculate the network distance from our point to the next closest line
# 2. keep the snapped point for that id that was closest *along the network*  to the previous id
# 3. set the newly snapped point as our the point
# 4. repeat
  
# get distances from our reference point to the next point id
for(i in nearest_road$id : (nearest_road$id-1)){
  #afairw to idi iparxon layer kai psaxnw xwris maxDist
  line <- line - nearest_road$LAYER 
  new_road <- nearest_line(....)
  next_dist <- which.min(abs(nearest_road[nearest_road$Accident_Index == i ,]$snap_dist - new_road[new_road$Accident_Index == i,]$snap_dist ))
  next_option <- nearest_road[nearest_road$id == i +1,]
...
count(mergedDf[mergedDf$Date >= "2019-01-01" & mergedDf$Date <= "2019-06-31" ,])
```
...
edin_road_data_3 <- mergedDf[,-c(36,37,41,42,44)]
edin_road_data_3 <- rename(edin_road_data_3,c("Longitude.before" = "Longitude", "Latitude.before" = "Latitude")) 

edin_zone_rd <- rename(edin_zone_rd, c("1st_Road_Number" = "X1st_Road_Number", "2nd_Road_Number" = "X2nd_Road_Number", 
                                       "1st_Road_Class" = "X1st_Road_Class", "2nd_Road_Class" = "X2nd_Road_Class",
...
edin_zone_data <- rbind.fill(edin_zone_rd,edin_road_data_3)
data <- edin_zone_data
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
total_zones <- buffer_data(data,edin_impl_zones)
```
...
control_zone_data1 <- rbind.fill(control_zone_rd,control_zone_data)
...
final_df <- final_df[,-c(1,36,41,42,47:54)]
head(final_df[,c(1,38,39,40)])
