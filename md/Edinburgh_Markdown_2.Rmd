...
mergedDf1 <- merge(data.frame(nearest_road),edin_cons_streets@data,by.x = "nearest_line_id" ,by.y = "ID")
#mergedDf1$Accident_Index <- as.character(mergedDf1$Accident_Index)
casualties_data$Accident_Index <- as.character(casualties_data$Accident_Index)
casualties_data <- rbind.fill(casualties_data,rd_2019)
casualties_data <- casualties_data[,-c(3:8)]
mergedDf <- merge(mergedDf1,casualties_data)
...
count(mergedDf1[mergedDf1$year == "2013", ])
...
s <-sum(pre[pre$LAYER == "30mph",2],pre[pre$LAYER == "20mph existing streets",2],s2,pre[pre$LAYER == "40mph",2])
summ <-sum(post[post$LAYER == "30mph",2],post[post$LAYER == "20mph existing streets",2],s2,post[post$LAYER == "40mph",2])

type <- c("Streets at 30mph","Streets at 20mph","Streets at 30mph changed to 20mph","Streets at 40mph","All streets")
collisions_pre_20mph <- c(pre[pre$LAYER == "30mph",2],pre[pre$LAYER == "20mph existing streets",2],s1,pre[pre$LAYER == "40mph",2],s)
collisions_post_20mph <- c(post[post$LAYER == "30mph",2],post[post$LAYER == "20mph existing streets",2],s2,post[post$LAYER == "40mph",2],summ)
rate_pre_20mph <- round(collisions_pre_20mph/s,2)
rate_post_20mph <- round(collisions_post_20mph/summ,2)
...
summ2 <-sum(p[p$Speed_limit == 20,2],p[p$Speed_limit == 30,2],p[p$Speed_limit == 40,2])
summ3 <-sum(po[po$Speed_limit == 20,2],po[po$Speed_limit == 30,2],po[po$Speed_limit == 40,2])

collisions_pre_20mph <- c(p[p$Speed_limit == 20,2],p[p$Speed_limit == 30,2],p[p$Speed_limit == 40,2],summ2)
collisions_post_20mph <- c(po[po$Speed_limit == 20,2],po[po$Speed_limit == 30,2],po[po$Speed_limit == 40,2],summ3)
rate_pre_20mph <- round(collisions_pre_20mph/summ2,2)
rate_post_20mph <- round(collisions_post_20mph/summ3,2)
diff_in_rates <- rate_pre_20mph - rate_post_20mph
