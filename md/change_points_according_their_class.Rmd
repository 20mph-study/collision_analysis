...
changed_layer <- function(data1,edin_cons_streets){
  ...
  #Trunk road
  network_20mphlocal <- edin_cons_streets[edin_cons_streets@data$LAYER  %in% c( "30mph","20mph local streets", "20mph main streets","20mph existing streets","Part time 20mph"),]
  mergedDf <- delete_na(data1[data1$LAYER == c('Trunk roads'), ],"Accident_Index")
  new_lines_20mphlocal <- nearest_line(mergedDf,network_20mphlocal,12)
  new_nearest_road_20mphlocal <- spTransform(new_lines_20mphlocal,"+init=epsg:4326")
  df5 <- unique(merge(data.frame(new_nearest_road_20mphlocal),network_20mphlocal@data,by.x = "nearest_line_id" ,by.y = "ID"))
  return(unique(rbind.fill(df1,df2,df3,df4,df5)))
}
...
intersectiondf <- changed_layer(final[,-c(42:46)],edin_cons_streets)[,-c(1,37,40,41,43,46,47,49)]

#For these collisions find the road class 
new_class <- new_nearest_line(intersectiondf,inter,15)
...
new_class_2019 <- new_class[new_class$year == "2019", ]
new_class_2019[new_class_2019$CODE == 3001 ,]$X1st_Road_Class <- "A"
new_class_2019[new_class_2019$CODE == 3002 ,]$X1st_Road_Class <- "B"
new_class_2019[new_class_2019$CODE == 3004 ,]$X1st_Road_Class <- "Unclassified"
new_class <- rbind.fill(new_class[new_class$year != "2019", ],new_class_2019)[,-c(42:45,47,50:54)]


#aggregate(Accident_Index~X1st_Road_Class + LAYER.x ,new_class,length)
#aggregate(Accident_Index~X1st_Road_Class + LAYER.y ,new_class,length)
```

```{r}
changed <- delete_na(new_class,"LAYER.y")
changed$LAYER.x <- as.character(changed$LAYER.x)
changed$LAYER.y <- as.character(changed$LAYER.y)

changed$LAYER <- changed$LAYER.x
changed[changed$X1st_Road_Class == "A" & changed$LAYER.y %in% c("20mph main streets","Trunk roads","30mph","Part time 20mph"),]$LAYER <- 
  changed[changed$X1st_Road_Class == "A" & changed$LAYER.y %in% c("20mph main streets","Trunk roads","30mph","Part time 20mph"),]$LAYER.y

changed[changed$X1st_Road_Class == "Unclassified" & changed$LAYER.y %in% c("20mph local streets","20mph existing streets"),]$LAYER <- 
  changed[changed$X1st_Road_Class == "Unclassified" & changed$LAYER.y %in% c("20mph local streets","20mph existing streets"),]$LAYER.y
...
results <- rbind.fill(not_changed,changed)
```
...
road_trunk <- filter(edin_streets, edin_streets$LAYER == "Trunk roads")
 ...
 leaflet() %>% addTiles() %>%addPolylines(data=inter_c)  %>% addLegend("bottomright",colors =c("#41ab5d",  "#fc4e2a", "#4eb3d3",  "#08306b","#6a51a3","#ec7014"),
  ...
  addPolylines(data = road_existing_20, color= "#41ab5d", group = 'existing 20mph' ) %>%
    ...
    addPolylines(data = road_part,color= "#6a51a3" , group = 'part time 20mph')%>%
    addPolylines(data = road_trunk,color= "#ec7014", group = 'Trunk roads') %>%
    addLayersControl(overlayGroups = c( 'existing 20mph','local 20mph','main 20mph','30mph','part time 20mph',"Trunk roads"),options = layersControlOptions(collapsed = FALSE))
```
