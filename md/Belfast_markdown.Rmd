...
#belf_data %>% group_by(year) %>% summarise(nr = n())
...
nearest_line_sp <- localSnapPointsToLines(df, road_net, maxDist=10, withAttr = T)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
pre_20 <- count_pre(merged[merged$year >="2013" & merged$year <= "2015", ])
pre_20
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
count(merged[merged$year >="2018" & merged$year <= "2019", ])
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
type <- c("Streets changed to 20mph"," Existing streets,not changed")
collisions_pre_20mph <- c(234,3785)
collisions_post_20mph <- c(86,1426)
rate_pre_20mph <- c("0.058" ,"0.942")
rate_post_20mph <- c("0.057" ,"0.943")
diff_in_rates <- c("0.001","-0.001")
percentage_diff_in_rates <- c("0.1 %" ,"-0.1 %")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
total <- total_zones[total_zones$a_date >="2018-01-01" & total_zones$a_date <= "2018-12-31", ]
leaflet(belf_impl_zones_out) %>% addTiles() %>% addPolygons() %>%
                         setView(lng = -5.926437 , lat = 54.607868, zoom = 14) %>% 
                          addMarkers(lng = total$X, lat = total$Y, 
                                      popup = paste("Accident index: ", total$a_ID, "<br>",
...
write.csv(updated_dataset, file ="V:\\Studies\\MOVED\\HealthImpact\\Data\\20mph study collisions\\final_Belfast_data.csv",row.names=FALSE)
```
