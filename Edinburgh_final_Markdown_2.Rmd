...
nearest_line <-function(df,road_net){
  ...
  nearest_line_sp <- snapPointsToLines(df,road_net,maxDist=6)
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
pre_20 <- count_pre(mergedDf[mergedDf$year == c("2013","2014","2015","2016"), ])
```

>Count post 20mph

```{r,echo=FALSE,warning=FALSE,message=FALSE}
count(mergedDf[mergedDf$year == c("2018","2019"), ])
```
...
#collisions_pre_20mph <- c("610","336","1325","198","2469")
#collisions_post_20mph <- c("158","71","290","51","570")
#rate_pre_20mph <- c("0.247","0.136","0.537","0.080","1")
#rate_post_20mph <- c("0.277","0.125","0.509","0.089","1")
#diff_in_rates <- c("0.03","-0.011","-0.028","0.009","0")
#percentage_diff_in_rates <- c("3 %" ,"-1.1 %","2.8 %","0.9 %","0 ")
...
#collisions_pre_20mph <- c("98","2232","194","2524")
#collisions_post_20mph <- c("316","211","57","584")
#rate_pre_20mph <- c("0.039","0.884","0.077","1")
#rate_post_20mph <- c("0.541","0.361","0.098","1")
#diff_in_rates <- c("0.502","-0.523","0.021","0")
#percentage_diff_in_rates <- c("50.2 %" ,"52.3 %","2.1 %","0 ")
#df <- data.frame(type,collisions_pre_20mph,collisions_post_20mph,rate_pre_20mph,rate_post_20mph,diff_in_rates,percentage_diff_in_rates)
...
leaflet(control_zone_1) %>% addTiles() %>% addPolygons() %>% addMarkers(lng = con_data$Longitude, lat = con_data$Latitude)
...
leaflet(control_zone_7) %>% addTiles() %>% addPolygons()%>% addMarkers(lng = con_data$Longitude, lat = con_data$Latitude)
...
leaflet(control_zone_5) %>% addTiles() %>% addPolygons()%>% addMarkers(lng = con_data$Longitude, lat = con_data$Latitude)
