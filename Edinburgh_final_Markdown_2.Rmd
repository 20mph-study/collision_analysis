...
#data_path <- "../data/Edin dataset.zip"
#shapefile_path <- "../data/Edin shapefiles.zip"
#
```
...
control_gdb_path <- paste0(dir_path, "20mph study collisions\\SG_DataZoneBdry_2011")
...
mergedDf <- unique(merge(data.frame(nearest_road),edin_cons_streets@data,by.x = "nearest_line_id" ,by.y = "ID"))
casualties_data <- unique(rbind.fill(casualties_data,rd_2019))
mergedDf <- unique(merge(mergedDf,casualties_data))
...
mergedDf <- unique(rbind.fill(acc_sev,rd_2019))
