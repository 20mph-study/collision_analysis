belf_annual_table <- function(total_zones,total,pre,post){
  ...
  sum2 <- c(nrow(total_zones[total_zones$a_date >= "2016-02-02" & total_zones$a_date <= "2019-03-31",]))
  
  #belcount_pre
  not_changed_pre <-  c(nrow(not_changed[not_changed$a_date >= "2013-02-01" & not_changed$a_date <= "2016-01-31",]))
  not_changed_post <- c(nrow(not_changed[not_changed$a_date >= "2016-02-02" & not_changed$a_date <= "2019-03-31",]))
