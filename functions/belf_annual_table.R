belf_annual_table <- function(belf_data,pre,post){
  ...
  sum1 <- c(nrow(belf_data[belf_data$a_date >= "2013-02-01" & belf_data$a_date <= "2016-01-31",]))
  sum2 <- c(nrow(belf_data[belf_data$a_date >= "2016-02-02" & belf_data$a_date <= "2019-03-31",]))
  
  #belcount_pre
  not_changed <- belf_data
  pre_not_changed <- belcount_pre(not_changed[not_changed$a_date >= "2013-02-01" & not_changed$a_date <= "2016-01-31",] )
  post_not_changed <- belcount_pre(not_changed[not_changed$a_date >= "2016-02-02" & not_changed$a_date <= "2019-03-31",] )
  ...
  rate_post_20mph <- round(collisions_post_20mph / 2.16, 2)
  ...
  colnames(df)=c("Type", "collisions pre-20mph (2013 / 2016 - 3 years)", "collisions post-20mph (2016 / 2019 - 26 months)", "rate pre-20mph", "rate post-20mph","diff in rates = pre - post", "Perc.diff.rates = (pre - post) / pre * 100")
