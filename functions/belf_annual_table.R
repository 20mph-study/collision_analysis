belf_annual_table <- function(total_zones,total,pre,post){
  ...
  rate_post_20mph <- round(collisions_post_20mph / 3.166, 2)
  ...
  colnames(df)=c("Type", "collisions pre-20mph (2013 / 2016 - 3 years)", "collisions post-20mph (2016 / 2019 - 3years & 2months)", "rate pre-20mph", "rate post-20mph","diff in rates = pre - post", "Perc.diff.rates = (pre - post) / pre * 100")
