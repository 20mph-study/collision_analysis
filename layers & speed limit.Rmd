...
new_lines_existing <- nearest_line(mergedDf,network)
new_nearest_road <- spTransform(new_lines,"+init=epsg:4326")
new_nearest_road <-new_lines@data
new_nearest_road <- unique(merge(new_nearest_road,network@data,by.x = "nearest_line_id" ,by.y = "ID"))
...
network_30mph <- edin_cons_streets[edin_cons_streets@data$LAYER  %in% c( "20mph local streets","20mph main streets","30mph","Part time 20mph"),]

new_lines_30mph <- nearest_line(mergedDf,network)
new_nearest_road_30mph <- spTransform(new_lines,"+init=epsg:4326")
new_nearest_road_30mph <-new_lines@data
